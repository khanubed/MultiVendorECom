  ...
  const vendors = [
    {
      ...
      description: "Specializing in high-velocity logistics and supply chain optimization components for global markets.",
      images: [
    ...
    },
    {
      ...
      description: "Advanced monitoring and analytics infrastructure for real-time market data visualization and processing.",
      images: [
    ...
    },
    {
      ...
      description: "Premium freight and last-mile fulfillment infrastructure specialized for delicate high-value technical assets.",
      images: [
  ...
  return (
    <section className="mb-xl">
      <div className="flex items-center gap-md mb-lg">
        ...
        <h2 className="text-headline-lg font-headline-lg text-primary px-md">Vendor Spotlights</h2>
        <div className="h-px bg-outline-variant flex-1"></div>
      </div>

      <div className="grid grid-cols-1 md:grid-cols-3 gap-lg">
        {vendors.map((vendor, index) => (
          <div key={index} className="premium-card p-lg border-2 border-transparent hover:border-secondary transition-all flex flex-col justify-between">
            <div>
              <div className="flex items-center gap-md mb-lg">
                <div className={`w-16 h-16 rounded-2xl flex items-center justify-center ${vendor.iconStyle}`}>
                  {vendor.icon}
                </div>
                <div>
                  <h3 className="font-bold text-xl">{vendor.name}</h3>
                  <div className="flex items-center text-secondary text-sm font-medium gap-1">
                    <ShieldCheck className="w-4 h-4 text-secondary" /> {vendor.badge}
                  </div>
                </div>
              </div>
              <p className="text-on-surface-variant text-body-md mb-lg">{vendor.description}</p>
              
              <div className="grid grid-cols-3 gap-2 mb-lg">
                {vendor.images.map((imgSrc, imgIdx) => (
                  <div key={imgIdx} className="aspect-square bg-surface-container rounded-lg overflow-hidden">
                    <img className="w-full h-full object-cover" src={imgSrc} alt={`Asset portfolio component ${imgIdx + 1}`} />
                  </div>
                ))}
              </div>
            </div>
            <button className="w-full py-2 bg-surface-container-high text-primary rounded-lg font-bold hover:bg-secondary hover:text-white transition-all text-label-md font-label-md">
              {vendor.actionText}
            </button>
          </div>
        ))}
